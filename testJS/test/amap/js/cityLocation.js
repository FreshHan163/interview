...
//实现把各市加到自己所属的省里，生成一个provinceObj{cities[0]{code,label,lat,lng,name},label,name,lat,lng,code}
...
console.log(provinceObj.length);
// console.log(JSON.stringify(provinceObj));
    var newObj = JSON.stringify(provinceObj);
    for(var i in newObj) {
        if(newObj[i].length != undefined) {
...
/*
    for(var i=0; i<newObj.length; i++) {
        // console.log(newObj[i]);
        if(c == newObj[i].label) {
            // console.log(provinceObj[i].lng);
            // console.log(provinceObj[i].lat);
            return;
        } else {
            // console.log("error");
        }
...
//引入地图
mapObj = new AMap.Map("map", {center:new AMap.LngLat(116.397428,39.90923), resizeEnable:true}); //新建一个地图示例
...
mapObj.plugin(["AMap.ToolBar","AMap.OverView", "AMap.PlaceSearch","AMap.CitySearch","AMap.Geocoder"], function(){
    ...
    placeSearch = new AMap.PlaceSearch;     //一个新的插件
    AMap.event.addListener(placeSearch, "complete", citySearchCallback);
...
//按关键字进行搜索
var citySearchCallback = function(result){
        console.log("dd");
        console.log(result);
        if(result == 'complete'){
            return
        ...
        if(result.poiList && result.poiList.pois && result.poiList.pois.length){
            var poi = result.poiList.pois[0];       //输出第一个值
            // mapObj.setZoomAndCenter(13, poi.location);  //设置中心区域
            // marker.setTitle([poi.name, poi.address].join(poi.name && poi.address ? "\n" : ""));
            // marker.setPosition(poi.location);
            // marker.setMap(mapObj);
            // $txtCoordinate[0].value = poi.location.toString();  //输出坐标
            console.log(poi.location.toString());       //输出坐标
        }
        else{
            alert("没有搜索到结果！！！");
        }
...
var cityname = ['北京','深圳','广州'];
for( i in cityname) {
    if(cityname[i]) {

        // console.log(i);
        // console.log(typeof cityname[i]);
        placeSearch.search(cityname[i], citySearchCallback);
